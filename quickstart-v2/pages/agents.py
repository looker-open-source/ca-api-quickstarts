...
from state import refresh_agents_state
        ...
        for ag in st.session_state.agents:
            ...
            with st.expander(f"**{name}**"):
                ...
                with col1:
                    ...
                    st.write(f"**DisplayName:** {ag.display_name}")
                    st.write(f"**Description:** {ag.description}")
                    st.write(f"**Created:** {ag.create_time}")
                    st.write(f"**Updated:** {ag.update_time}") 
                with col2:
                    with st.popover("**System instructions:**"):
                        st.write(ag.data_analytics_agent.published_context.system_instruction)
                    with st.popover("**Data source:**"):
                        st.write(ag.data_analytics_agent.published_context.datasource_references)
